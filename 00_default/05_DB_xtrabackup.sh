...
if (( "$(df -h | grep -i "/sj_dir_ldf" | awk '{print $(NF-1)}'  | sed 's/%//g')" >= "90" )) ; then exit 1 ; fi
...
if [ ! -d ${dump_dir}/inc1/${dump_date} ]; then
        echo "[${log_date}]_info_sj_dir xtrabackup -u ${DB_user} -p --backup  --target-dir=${dump_dir}/inc1/${dump_date} --incremental-basedir=${dump_date}/full/${inc1_date}-00 " >> /var/log/xtrabackup-inc1.log
...
exit 0
if (( "\$(df -h | grep -i "/sj_dir_mdf" | awk '{print \$(NF-1)}'  | sed 's/%//g')" >= "90" )) ; then echo "/sj_dir_mdf directory 90% up!" ; fi
if [ ! -d "/sj_dir_ldf/sjyun" ]; then mkdir -p /sj_dir_ldf/sjyun ;fi
...
if [ -d "/sj_dir_mdf/MySQL" ]; then
        if [ -n "\$(lsof +D /sj_dir_mdf/MySQL)" ]; then exit 1 ; lsof +D /sj_dir_mdf/MySQL ; fi
fi;
if [ -d "/sj_dir_mdf/MySQL" ]; then mv -b /sj_dir_mdf/MySQL/ /sj_dir_mdf/MySQL_XtraBK_\${bk_log_date}/ ; else echo "The directory /sj_dir_mdf/MySQL does not exist." ;fi
...
rsync -av \${restore_dir}/full/\${restore_full_date}/ /sj_dir_ldf/sjyun/\${restore_full_date}_ORG_\${bk_log_date}/
rsync -av \${restore_dir}/inc1/\${restore_date}/ /sj_dir_ldf/sjyun/\${restore_date}_ORG_\${bk_log_date}/
...
if [ "\$?" -eq "0" ] ; then chown -R mysql.mysql /sj_dir_mdf/MySQL ; chown -R mysql.mysql /sj_dir_ldf/MySQL ;fi
