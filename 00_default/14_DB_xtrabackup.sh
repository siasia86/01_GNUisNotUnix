...
check_disk_space() {
	if (( $(df -h | sed 's/%//g' | awk '/\/sjdir_ldf/ { print $(NF-1) >= 90 }') )); then
		echo "/sjdir_ldf directory 90% up."
		exit 1
	fi
}
...
perform_incremental_backup() {
	if [ ! -d ${dump_dir}/inc1/${dump_date} ]; then
		echo "[${log_date}]_info_sjdir xtrabackup -u ${DB_user} -p --backup  --target-dir=${dump_dir}/inc1/${dump_date} --incremental-basedir=${dump_dir}/full/${inc1_date}-00 " >> /var/log/xtrabackup-inc1.log
...
# Function to check disk space
check_disk_space() {
	if (( \$(df -h | sed 's/%//g' | awk '/\/sjdir_mdf/ { print \$(NF-1) >= 90 }') )); then
		echo "/sjdir_mdf directory 90% up."
		exit 1
	fi
...
create_directory() {
	...
	if [ ! -d "/sjdir_ldf/infra" ]; then
		mkdir -p /sjdir_ldf/infra
	fi
}
...
move_existing_directory() {
	if [ -d "/sjdir_mdf/MySQL/" ]; then
		if [ -n "\$(lsof +D /sjdir_mdf/MySQL)" ]; then
			lsof +D /sjdir_mdf/MySQL
			exit 5
		fi
		mv -b /sjdir_mdf/MySQL/ /sjdir_mdf/MySQL_XtraBK_\${bk_log_date}
	else
		echo "The directory /sjdir_mdf/MySQL/ does not exist."
	fi
...
perform_rsync() {
	local source_dir=\$1
	local backup_date=\$2
	rsync -av "\${source_dir}/\${backup_date}/" "/sjdir_ldf/infra/\${backup_date}_ORG_\${bk_log_date}/" || { echo "\$?" ; exit 6; }
}
...
perform_xtrabackup_restore() {
	...
	if [ "\$?" -eq "0" ]; then
		chown -R mysql.mysql /sjdir_mdf/MySQL
		chown -R mysql.mysql /sjdir_ldf/MySQL
	fi
